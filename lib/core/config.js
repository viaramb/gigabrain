  ...
  properties: {
    ...
    recall: {
      ...
      properties: {
        ...
        semanticRerankEnabled: { type: 'boolean', default: false },
        semanticRerankAlpha: { type: 'number', default: 0.7 },
...
const normalizeConfig = (rawConfig = {}, options = {}) => {
  ...
  const out = {
    ...
    recall: {
      ...
      semanticRerankEnabled: merged?.recall?.semanticRerankEnabled === true,
      semanticRerankAlpha: Math.max(0, Math.min(1, Number(merged?.recall?.semanticRerankAlpha ?? 0.7) || 0.7)),
