...
const buildGigabrainCliResolverSnippet = ({
  ...
  const packageRootHintEsc = shellEscape(packageRootHint ? path.resolve(packageRootHint) : '');
  return `
...
run_gigabrain_cli() {
  ...
  echo "Tried repo-local node_modules/.bin, command -v, npx --no-install, and the setup-time source hint." >&2
