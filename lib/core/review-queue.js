...
// Allowed row properties whitelist for secure deserialization
const ALLOWED_ROW_PROPERTIES = new Set([
  'timestamp', 'status', 'reason', 'reason_code', 'action', 'payload',
  'queued_at', 'created_at', 'updated_at', 'id', 'memory_id', 'excerpt',
  'target', 'target_memory_id', 'candidates', 'suggested_commands', 'run_id',
  'requested_action', 'scope', 'content', 'type', 'confidence', 'durability',
  'value_score', 'value_label', 'source', 'source_path', 'source_kind',
]);
...
const sanitizeRow = (row) => {
  ...
  for (const key of Object.keys(row)) {
    ...
    if (ALLOWED_ROW_PROPERTIES.has(key)) {
      const value = row[key];
      // Block prototype pollution attempts
      if (key === '__proto__' || key === 'constructor' || key === 'prototype') {
        continue;
      }
      // Recursively sanitize nested objects (like payload)
      if (value && typeof value === 'object' && !Array.isArray(value)) {
        sanitized[key] = sanitizeRow(value);
      } else if (Array.isArray(value)) {
        sanitized[key] = value.map((item) =>
          item && typeof item === 'object' ? sanitizeRow(item) : item
        );
      } else {
        sanitized[key] = value;
      }
