...
const VALID_ACTION_RE = /^(remember|update|replace|forget|protect|do_not_store)$/;
const VALID_TYPE_RE = /^(USER_FACT|PREFERENCE|DECISION|ENTITY|EPISODE|AGENT_IDENTITY|CONTEXT)$/;
const VALID_SCOPE_RE = /^[a-zA-Z0-9_:.\-]{1,256}$/;
const VALID_MEMORY_ID_RE = /^[a-zA-Z0-9\-]{1,128}$/;

const validateAction = (action) => VALID_ACTION_RE.test(String(action || ''));
const validateType = (type) => !type || VALID_TYPE_RE.test(String(type || ''));
const validateScope = (scope) => !scope || VALID_SCOPE_RE.test(String(scope || ''));
const validateMemoryId = (id) => !id || VALID_MEMORY_ID_RE.test(String(id || ''));

const shellEscapeSingle = (value = '') => {
  const str = String(value || '');
  // Block any characters that could be used for command injection
  if (/[;&|`$(){}[\]!\\]/.test(str)) {
    // Return empty string for potentially dangerous input
    return '';
  }
...
const buildSuggestedCommands = ({ action = {}, scope = '', candidates = [] } = {}) => {
  ...
  if (!validateAction(action.action)) return [];
  if (!validateScope(scope)) return [];
  if (!validateType(action.type)) return [];
  if (action.target_memory_id && !validateMemoryId(action.target_memory_id)) return [];

  const content = String(action.content || '').trim();
  const base = ['node scripts/gigabrainctl.js control apply'];
  base.push(`--action ${String(action.action || '').trim()}`);
  if (scope) {
    const escapedScope = shellEscapeSingle(scope);
    if (escapedScope) base.push(`--scope '${escapedScope}'`);
  }
  if (action.type) base.push(`--type ${String(action.type || '').trim()}`);
  if (action.confidence != null && String(action.confidence).trim()) {
    const conf = String(action.confidence).trim();
    ...
    if (Number.isFinite(confNum) && confNum >= 0 && confNum <= 1) {
      base.push(`--confidence ${conf}`);
    }
  }
  if (content) {
    const escapedContent = shellEscapeSingle(content);
    if (escapedContent) base.push(`--content '${escapedContent}'`);
  }
  const commands = [];
  for (const row of candidates.slice(0, 3)) {
    const memoryId = String(row.memory_id || row.id || '').trim();
    if (!memoryId || !validateMemoryId(memoryId)) continue;
    commands.push(`${base.join(' ')} --target-memory-id ${memoryId}`);
  }
  if (commands.length === 0 && action.target) {
    const escapedTarget = shellEscapeSingle(String(action.target || ''));
    if (escapedTarget) {
      commands.push(`${base.join(' ')} --target '${escapedTarget}'`);
    }
  }
  return commands;
};
