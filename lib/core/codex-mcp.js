...
const SERVER_VERSION = '0.5.1-codex';
...
const createMcpServer = (defaults = {}) => {
  ...
  server.registerTool('gigabrain_recall', {
    description: 'Recall Gigabrain memories for the current Codex workspace. Use target=user for stable personal memory and target=project for repo-specific continuity.',
    inputSchema: {
  ...
  server.registerTool('gigabrain_checkpoint', {
    description: 'Write a native-only Codex App session checkpoint into today\'s Gigabrain daily log.',
    inputSchema: {
