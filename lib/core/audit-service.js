...
const shouldRunLlmReview = ({
  ...
  return score >= clamp01(llmConfig?.minScore ?? 0.24) && score <= clamp01(llmConfig?.maxScore ?? 0.52);
};
...
const runAudit = async ({
  ...
  try {
    ...
    const semanticMap = buildSemanticMap(activeRows);
    ...
    const llmCfg = {
      ...
      minScore: Number(llm.minScore ?? config?.llm?.review?.minScore ?? 0.24),
      maxScore: Number(llm.maxScore ?? config?.llm?.review?.maxScore ?? 0.52),
      minConfidence: Number(llm.minConfidence ?? config?.llm?.review?.minConfidence ?? 0.7),
    ...
    for (const memory of activeRows) {
      ...
      if (semantic && Number(semantic.similarity) >= Number(config?.dedupe?.autoThreshold ?? 0.92)) {
      ...
      if (canLlmReview) {
        ...
        if (llmResult.ok) {
          if (Number(llmResult.confidence || 0) >= Number(llmCfg.minConfidence || 0.7) && llmResult.decision) {
      ...
      classified.push({ memory, deterministic, semantic, action, reasons });
    }

    // Phase 2: apply all results inside a single transaction
    db.exec('BEGIN');
    try {
      for (const { memory, deterministic, semantic, action, reasons } of classified) {
